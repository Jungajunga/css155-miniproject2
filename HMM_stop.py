...
#### on unsupervised learning, implement the stopping conditions
#### When we do learning,  we use this file. 
...
class HiddenMarkovModel:
    ...
    def unsupervised_learning(self, X,threshold):
        '''
        ...
        while(1):
            ...
            for curr in range(self.L):
                for nxt in range(self.L):
                    A[curr][nxt] = A_num[curr][nxt] / A_den[curr]

            for curr in range(self.L):
                for xt in range(self.D):
                    O[curr][xt] = O_num[curr][xt] / O_den[curr]
            
            norms = np.linalg.norm(np.subtract(self.A , A)) + np.linalg.norm(np.subtract(self.O , O))
            print(norms)
            # now update
            if norms < threshold: 
                break
...
def unsupervised_HMM(X, n_states, threshold):
    '''
    ...
    observations = [i for i in range(3232)]
    ...
    HMM.unsupervised_learning(X, threshold)
